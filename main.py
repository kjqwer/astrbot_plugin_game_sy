...
class HuanJuPlugin(星标):
    ...
    async def hit(self, event: AstrMessageEvent):
        '''要牌'''
        group_id = event.message_obj。group_id
        if not group_id 或 group_id not in self.game_rooms:
        ...
        if room["status"] != "playing" 或 player_id != room["current_player"]:
